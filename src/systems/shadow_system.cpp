...
void shadow_system_update(app_t* app, float dt, void* udata, Transform* transforms, Animator* animators, BoardPiece* board_pieces, Shadow* shadows, int entity_count)
{
	for (int i = 0; i < entity_count; ++i) {
		...
		if (shadow->visible) {
			...
			tx.p.y -= 7.0f;
			if (shadow->small) {
				shadow_sprite_small.transform = tx;
				shadow_sprite_small.draw(batch);
			} else if (shadow->tiny) {
				shadow_sprite_tiny.transform = tx;
				shadow_sprite_tiny.draw(batch);
			} else if (shadow->big) {
				shadow_sprite_big.transform = tx;
				shadow_sprite_big.draw(batch);
			} else {
				shadow_sprite.transform = tx;
				shadow_sprite.draw(batch);
			}
		}
	}
}
